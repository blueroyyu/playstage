...
class _MainViewState extends State<MainView> {
  ...
  void _initSendBird() async {
    ...
    try {
      ...
      if (memberResponse['resultCode'] == '200') {
        ...
        var profileUrl = '$s3Url/$userId/profile/${sd.owner!.tbMemberPhotoInfoList?.first.photoSavedFileName}';
        User sbUser = await _connect(userId, sd.owner!.nickName ?? sd.owner!.memberName!);
        if (kDebugMode) {
          print(sbUser.toJson());
        }

        sendbird.updateCurrentUserInfo(nickname: sd.owner!.nickName ?? sd.owner!.memberName, fileInfo: FileInfo.fromUrl(url: profileUrl));
      }
                          ...
                          children: [
                            Positioned.fill(
                              child: AnimatedSwitcher(
                                ...
                                child: Image.network(
                                  member.makeProfileImagePath(index: _currentProfileIndex),
                            ...
                            member.tbMemberPhotoInfoList!.length > 1
                                ? Positioned(
                                    ...
                                    right: 0,
                                    child: DotsIndicator(
                                      dotsCount: member.tbMemberPhotoInfoList!.length, // 전체 페이지 수
                                            ...
                                            children: [
                                              Expanded(
                                                ...
                                                child: ElevatedButton(
                                                  ...
                                                  onPressed: () async {
                                                    await _requestHate(_currentMember!.memberId!);
                                              ...
                                              Expanded(
                                                ...
                                                child: ElevatedButton(
                                                  ...
                                                  onPressed: () async {
                                                    final matched = await _requestLike(_currentMember!.memberId!);

                                                    if (matched) {
                                                      Get.to(() => MatchedView(matchedMember: _currentMember!));
                                                    } else {
                                                      _pageController.nextPage(duration: const Duration(milliseconds: 500), curve: Curves.linear);
                                                    }
                                                  },
    ...
    setState(() {
      if (_currentMember != null) {
        if (_currentMember!.tbMemberPhotoInfoList!.isNotEmpty) {
          _currentProfileIndex = (_currentProfileIndex + 1) % _currentMember!.tbMemberPhotoInfoList!.length;
        }
      }
    });
  }
  ...
  Future<bool> _requestLike(String targetMemeberId) async {
    try {
      var responseData = await ApiProvider.requestToggleMemberLike(SharedData().owner!.memberId!, targetMemeberId);
      if (responseData['resultCode'] == '200') {
  ...
  Future<void> _requestHate(String targetMemeberId) async {
    try {
      await ApiProvider.requestAddHateMember(SharedData().owner!.memberId!, targetMemeberId);
    } on Exception catch (e) {
      if (kDebugMode) {
        print(e);
      }
        ...
        child: Row(
          ...
          children: [
            ...
            Container(
              ...
              child: IconButton(
                onPressed: () {
                  Get.to(() => ConnectionView(memberId: sd.owner!.memberId!), transition: Transition.fadeIn);
                },
                icon: Icon(index == 1 ? CupertinoIcons.star_fill : CupertinoIcons.star),
            ...
            Container(
              ...
              child: IconButton(
                onPressed: () {
                  index == 2
                      ? Get.to(() => const AddFeed(), transition: Transition.fadeIn)
                      : Get.to(() => FeedView(member: sd.owner!), transition: Transition.fadeIn);
                },
                icon: index == 2 ? const Icon(CupertinoIcons.add_circled) : const Icon(CupertinoIcons.globe),
            ...
            Container(
              ...
              child: IconButton(
                onPressed: () {
                  Get.to(() => const ChannelListView(), transition: Transition.fadeIn);
                },
                icon: Icon(index == 3 ? CupertinoIcons.chat_bubble_fill : CupertinoIcons.chat_bubble),
            ...
            Container(
              ...
              child: IconButton(
                onPressed: () {
                  Get.to(() => const ProfileView(), transition: Transition.fadeIn);
                  // Get.to(() => MatchedView(matchedMember: SharedData().owner!));
                  // Get.to(() => const AppSettings());
                },
                icon: Icon(index == 4 ? CupertinoIcons.person_fill : CupertinoIcons.person),
